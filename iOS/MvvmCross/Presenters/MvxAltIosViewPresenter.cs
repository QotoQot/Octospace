...
{
#nullable enable
    public class MvxAltIosViewPresenter : MvxAttributeViewPresenter, IMvxAltIosViewPresenter
    {
        public MvxAltIosViewPresenter() { }

        // TODO: add/remove new windows from scene delegate or something
        UIWindow Window => UIApplication.SharedApplication.KeyWindow;

        // TODO: Dictionaries of <windowId, controller> for all below
        UINavigationController? MasterNavigationController { get; set; }

        UIViewController? PopoverViewController { get; set; }

        // dict of lists here
        List<UIViewController> ModalViewControllers { get; } = new List<UIViewController>();
        ...
        public override void RegisterAttributeTypes()
        {
            if (AttributeTypesToActionsDictionary == null)
                throw new InvalidOperationException("Can't register attribute types on null dictionary");
        ...
        protected virtual Task<bool> CloseRootViewController(IMvxViewModel viewModel, MvxRootPresentationAttribute attribute)
        {
            ...
            Dlog.Warning($"Ignored attempt to close the window root (ViewModel type: {viewModel.GetType().Name}");
        ...
        protected virtual bool TryCloseViewControllerInsideStack(UINavigationController navController, IMvxViewModel toClose, MvxChildPresentationAttribute attribute)
        {
            ...
            if (controllerToClose != null)
            {
                Guard.IsNotNull(controllers, nameof(controllers));
